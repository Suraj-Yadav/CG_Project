...
def loadModelFromFile(outputFile: str, context):
    ...
    for j in range(1, i + 1):
        for k in range(1, i + 1):
            ...
            bpy.data.objects[models[k - 1]].keyframe_insert(
                data_path="hide", index=-1, frame=j)

    return {"FINISHED"}


class vertexSelected(Operator):
    bl_idname = 'cgal.return_selected_vertices'
    bl_label = 'Return Vertices'
    ...
    def execute(self, context: Context):
        ...
        if obj == None:
            ...
            return {"CANCELLED"}
        context.scene.vertsSelected = selectFromMesh(obj).encode()
        return {"FINISHED"}
...
def register():
    ...
    bpy.utils.register_class(vertexSelected)
    ...
    bpy.types.Scene.inputFile = bpy.props.StringProperty(
        name="InputFile", subtype='FILE_PATH')
    bpy.types.Scene.executable = bpy.props.StringProperty(
        name="Executable", subtype='FILE_PATH')
    bpy.types.Scene.outputFile = bpy.props.StringProperty(
        name="OutputFile", subtype='FILE_PATH')
    bpy.types.Scene.vertsToSelect = bpy.props.StringProperty(
        name="Vertices", subtype='BYTE_STRING')
    bpy.types.Scene.edgesToSelect = bpy.props.StringProperty(
        name="Edges", subtype='BYTE_STRING')
    bpy.types.Scene.facesToSelect = bpy.props.StringProperty(
        name="Faces", subtype='BYTE_STRING')
    bpy.types.Scene.vertsSelected = bpy.props.StringProperty(
        name="Selected Vertices", subtype='BYTE_STRING')
    ...
    kmi = km.keymap_items.new(
        vertexModel.bl_idname, 'P', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        edgeModel.bl_idname, 'E', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        faceModel.bl_idname, 'F', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
...
def unregister():
    ...
    bpy.utils.unregister_class(vertexSelected)
    ...
    del bpy.types.Scene.vertsSelected
...
class cgalPanel(Panel):
    ...
    def draw(self, context: Context):
        ...
        row = layout.row()
        row.prop(context.scene, 'vertsSelected')
        row.operator("cgal.return_selected_vertices", icon="VERTEXSEL")

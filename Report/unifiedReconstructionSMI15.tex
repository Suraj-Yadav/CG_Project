...
\RETURN $false$
\ENDIF
\IF{$deg(edge) = 1$ $\wedge$ angle between this face and another face with same edge $< 90$}
\RETURN $false$
\ENDIF
\ENDFOR
\RETURN $true$
...
\STATE $updatePQ()$
\WHILE{$edgeCondtition \geq 0$}
\WHILE{$pq \neq \Phi$}
\STATE $face \leftarrow pq.pop()$
\IF{$deg(edge\in face) = 2$}
\STATE $continue$
\ENDIF
\STATE $count \leftarrow$ count of edges in $face$ longer than maxlength in current graph
\IF{$face\notin trianglesCovered$ $\wedge$ $validToAdd(face)$ $\wedge$ $count > edgeCondition$}
...
\begin{figure}[!h]
	\centering
	\subfigure[\label{fig:buddha1}]{
		\includegraphics[height=2.5in]{images/buddha1}
	}
\end{figure}
\begin{figure}[!h]
\centering
	\subfigure[\label{fig:buddha2}]{
		\includegraphics[height=2.5in]{images/buddha2}
	}
\end{figure}
\begin{figure}[!h]
\centering
	\subfigure[\label{fig:sheep1}]{
		\includegraphics[height=2.5in]{images/sheep1}
	}
\end{figure}
\begin{figure}[!h]
\centering
	\subfigure[\label{fig:sheep2}]{
		\includegraphics[height=2.5in]{images/sheep2}
	}
\end{figure}
\begin{figure}[!h]
\centering
    \subfigure[\label{fig:dragon}]{
		\includegraphics[height=2.5in]{images/dragon}
	}
	\caption{

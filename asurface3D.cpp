...
class SurfaceReconstruct {
	...
	double getTriScore(int u, int v, int w) {
		...
		double ab = AB.squared_length();
		double bc = BC.squared_length();
		double ac = AC.squared_length();

		// double minLength = std::min(std::min(ab,bc),ac);
		// double maxLength = std::max(std::max(ab,bc),ac);
		...
		// angA = abs(abs(AC*AB) - 0.5);
		// angB = abs(abs(AB*BC) - 0.5);
		// angC = abs(abs(BC*AC) - 0.5);

		// return  -std::max(std::max(angA,angB),angC);

		return score;//*(minLength/maxLength);
	}
